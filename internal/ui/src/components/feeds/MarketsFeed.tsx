...
export function MarketsFeed() {
  ...
  const [sort, setSort] = useState<SortField>('volume_num')
  ...
  // Fetch markets on q/status/sort/page change
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    setError(null)
    ...
    <div className="flex flex-col h-full gap-3">
      ...
      <div className="flex-1 overflow-hidden min-h-0">
        {loading ? (
          ...
          <ScrollArea className="h-full" viewportRef={eventsViewportRef}>
      ...
      {!loading && !error && (events.length > 0 || page > 1) && (
        <div className="shrink-0">
          <EventsPagination page={page} hasMore={hasMore} onPage={setPage} />
        </div>
      )}
    </div>
  )
    ...
    <Card className="py-4 gap-0">
      <CardHeader className="px-4 pb-2">
        ...
        <div className="grid grid-cols-3 gap-2 mt-2">
          ...
          {event.endDate && <Stat label="Ends" value={formatDate(event.endDate)} />}
        </div>
      </CardHeader>
      {markets.length > 0 && (
  ...
  return (
    <div className="rounded-md border bg-muted/30 px-3 py-2 space-y-1.5">
      <div className="flex items-start justify-between gap-2">
        ...
        >
          <Star className={`h-3.5 w-3.5 ${watched ? 'fill-yellow-400 text-yellow-400' : 'text-muted-foreground'}`} />
        </button>
      </div>
