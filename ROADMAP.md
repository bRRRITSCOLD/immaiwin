...
- **Parallel `for_each`** — opt-in via `data.parallelism > 1` (default 1 = sequential, back-compat). Body iterations run concurrently up to the cap (clamped to 32). Save-time validator refuses `parallelism > 1` with `on_error: stop` since in-flight goroutines can't be cleanly aborted mid-flight — authors pick either "stop on first fault sequentially" or "best-effort fan-out in parallel". Per-iteration wfCtx clone keeps named body-node writes from racing across iterations.
