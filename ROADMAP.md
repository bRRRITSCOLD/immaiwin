...
- **Multi-agent C — `fan_out` orchestrator primitive** — built-in `fan_out` tool registered on every agent. LLM calls it with `{calls: [{tool, args}, ...], parallelism?}` to dispatch a list of sub-tool calls concurrently against the agent's own catalog (built-ins, skills, as_tool nodes, sub-agents). Bounded by `parallelism` (default 5, cap 10). Returns `{results: [{output, error}]}` in input order; a single failure does NOT abort siblings so the LLM can pivot on partial success. Map-reduce over specialists in one tool call — no manual sequencing.
- **Parallel `for_each`** — opt-in via `data.parallelism > 1` (default 1 = sequential, back-compat). Body iterations run concurrently up to the cap (clamped to 32). `on_error: stop` semantics in parallel are SOFT: dispatch loop halts NEW iterations on first unsuppressed body error, in-flight goroutines run to completion (mid-flight cancel would leak partial side effects). `on_error: continue` is the best-effort variant — every item runs. Per-iteration wfCtx clone keeps named body-node writes from racing across iterations.
- **Per-iter agent checkpoint** — agent loop writes its mid-loop state (messages, iter index, usage, trace) to `WorkflowRun.PausedAgent` at the END of every ReAct iter via the new `CheckpointPausedAgent` lease-gated repo method. Worker death mid-loop → reclaim resumes at iter N+1 instead of restarting at iter 0 (existing `RunFromCheckpoint` already hydrates from `PausedAgent`). On successful agent completion the snapshot is cleared so a later worker-death reclaim doesn't try to resume an already-finished agent. Cost = 1 Mongo write per iter; best-effort (Mongo blip logs but doesn't kill the run; next iter's checkpoint overwrites).
