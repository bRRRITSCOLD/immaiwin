...
| `internal/api/handler` | `workflow_run_integration_test.go` | Workflow execution + OOB approval | `TestRun_TriggerOnly_PersistsSuccessRow`, `TestRun_NonexistentWorkflow_404`, `TestRun_Unauth_401`, `TestRunsList_FilteredByWorkflow` (workflow_id query param narrows list), `TestRun_NodeApproval_OOB_PendsThenResumesAfterApprove` (require_node_approval gate flips status to pending_approval, /approval POST resolves the gate via Redis, run resumes + completes) |
