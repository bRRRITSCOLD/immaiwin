...
| `internal/api/handler` | `workflow_run_integration_test.go` | Workflow execution + OOB approval (sessioned + magic-link redeem) | `TestRun_TriggerOnly_PersistsSuccessRow`, `TestRun_NonexistentWorkflow_404`, `TestRun_Unauth_401`, `TestRunsList_FilteredByWorkflow` (workflow_id query param narrows list), `TestRun_NodeApproval_OOB_PendsThenResumesAfterApprove` (require_node_approval gate → pending_approval → /approval POST resolves), `TestApprovalRedeem_BadToken_Returns401`, `TestApprovalRedeem_RunIDMismatch_Returns401` (token signed for run A → 401 against run B), `TestApprovalRedeem_HappyPath_ApproveResumesRun` (signed token → /approval/redeem → run resumes; audit ledger captures `approval_redeemed`), `TestApprovalRedeem_TokenSuperseded_Returns410` (replayed token after gate resolved is rejected) |
